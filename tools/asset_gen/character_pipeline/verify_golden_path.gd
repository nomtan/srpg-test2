...
func run() -> void:
	...
	for id in ["001", "002"]:
		var path := "res://assets/characters/generated/golden_path_%s/character.glb" % id
		...
		actor.configure("golden_" + id, "Face " + id, Vector2i(3 + actors.size() * 2, 3), "player")
		...
		if previous_skeleton:
			...
			for clip in ["idle", "walk", "attack", "hit"]:
				...
				for fraction in [0.0, .25, .5, .75, 1.0]:
					...
					check(matches, "001 animation reused by 002: %s %.2f" % [clip, fraction])
	...
	var center := (actors[0].position + actors[1].position) * .5 + Vector3.UP * .6
	...
	if "--capture" in OS.get_cmdline_user_args():
		...
		root.get_texture().get_image().save_png("res://artifacts/golden_path/godot_battle_distance.png")
		cam.size = 5
		for clip in ["idle", "walk", "attack", "hit"]:
			for actor in actors:
			...
			await process_frame
			await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://artifacts/golden_path/godot_" + clip + ".png")
	world.queue_free()
	await process_frame
	print("GOLDEN_PATH_VALIDATION: ", "FAILED" if failed else "PASSED")
