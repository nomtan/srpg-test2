"""Meshy hero2: fitted skin, authored in-place idle/walk/run, and Godot export.
...
ARTIFACTS = ROOT / 'artifacts/meshy_hero2'
GROUND = -0.950390100479126
TOP = 0.9458391070365906
RIG = 'Hero2_Rig'
MESH = 'Mesh_0'
...
def build():
    ...
    arm = bpy.data.armatures.new('Hero2_Skeleton')
    ...
    bone('neck', (0, 0, .045), (0, 0, .115), 'chest')
    bone('head', (0, 0, .115), (0, 0, .66), 'neck')
    for side, sign in [('L', 1), ('R', -1)]:
        bone('clavicle.' + side, (sign * .07, 0, .015), (sign * .30, 0, -.065), 'chest')
        bone('upper_arm.' + side, (sign * .30, 0, -.065), (sign * .435, -.025, -.23), 'clavicle.' + side)
        bone('forearm.' + side, (sign * .435, -.025, -.23), (sign * .565, -.055, -.35), 'upper_arm.' + side)
        bone('hand.' + side, (sign * .565, -.055, -.35), (sign * .60, -.08, -.445), 'forearm.' + side)
        # Hinge above the bulky kneecap so the plate clears the floor on push-off.
        bone('thigh.' + side, (sign * .19, -.015, -.44), (sign * .22, -.06, -.70), 'pelvis')
        bone('shin.' + side, (sign * .22, -.06, -.70), (sign * .22, -.005, -.87), 'thigh.' + side)
        bone('foot.' + side, (sign * .22, -.005, -.87), (sign * .22, -.16, -.92), 'shin.' + side)
        bone('toe.' + side, (sign * .22, -.16, -.92), (sign * .22, -.255, -.92), 'foot.' + side)
        bone('cape.' + side, (sign * .16, .19, -.025), (sign * .24, .29, -.42), 'chest')
        bone('cape_tip.' + side, (sign * .24, .29, -.42), (sign * .35, .42, -.83), 'cape.' + side)
    mode('OBJECT')
    ...
    modifier = mesh.modifiers.new('Hero2 Skin', 'ARMATURE')
    modifier.object = rig
    # Linear skinning matches the glTF/Godot deformation exactly.
    modifier.use_deform_preserve_volume = False
    rig['source_model'] = 'Meshy hero2: original 7242-vertex geometry, UVs and three packed PBR textures.'
...
def bind():
    ...
    for vertex in mesh.data.vertices:
        ...
        torso = blend(torso, {'neck': 1}, smooth(.005, .055, z))
        torso = blend(torso, {'head': 1}, smooth(.05, .115, z))

        arm = blend({'upper_arm.' + side: 1}, {'forearm.' + side: 1}, 1 - smooth(-.275, -.19, z))
        arm = blend(arm, {'hand.' + side: 1}, 1 - smooth(-.39, -.325, z))
        arm = blend(arm, {'clavicle.' + side: 1}, smooth(-.09, .015, z))
        boundary = .29 + .29 * max(0, -z)
        arm_amount = smooth(boundary - .025, boundary + .045, ax)
        arm_amount *= (1 - smooth(.07, .15, z)) * smooth(-.57, -.51, z)

        leg = blend({'thigh.' + side: 1}, {'shin.' + side: 1}, 1 - smooth(-.75, -.66, z))
        # Keep the protruding kneecap on the upper leg instead of folding its
        # lower lip beneath the boot as the short shin bends during recovery.
        kneecap = (1 - smooth(-.17, -.115, y)) * smooth(-.835, -.79, z) * (1 - smooth(-.68, -.63, z))
        leg = blend(leg, {'thigh.' + side: 1}, kneecap)
        leg = blend(leg, {'foot.' + side: 1}, 1 - smooth(-.875, -.825, z))
...
def animate():
    ...
    soles = {side: [v.co - rig.data.bones['foot.' + side].head_local for v in mesh.data.vertices
                    if v.co.x * sign > .07 and v.co.z < -.885 and v.co.y < .12]
             for side, sign in [('L', 1), ('R', -1)]}
    ...
    for clip, period in PERIODS.items():
        ...
        for frame in range(1, period + 2):
            ...
            for side, sign, offset in [('L', 1, 0), ('R', -1, .5)]:
                ...
                ankle = Vector((sign * .22, foot_y, GROUND - sole + lift))
...
def save_and_export():
    ...
    try:
        bpy.ops.export_scene.gltf(
            filepath=str(OUT / 'hero2.glb'), export_format='GLB', export_yup=True,
    ...
    print(json.dumps({'export': str(OUT / 'hero2.glb'), 'bytes': len(data), 'clips': clips, 'bones': 26, 'textures': 3, 'front': '+X', 'height': 2.2}, indent=2))


if __name__ == '__main__':
    bpy.ops.wm.open_mainfile(filepath=str(ARTIFACTS / 'hero2_before_rig.blend'))
    build()
    bind()
    animate()
    validate()
