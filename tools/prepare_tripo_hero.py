"""Preserve the Tripo hero source; build an Eevee toon rig and a matching game GLB.

blender --background --factory-startup --python tools/prepare_tripo_hero.py
...
OUT = ROOT / 'assets/characters/tripo_hiro'
ART = ROOT / 'artifacts/tripo_hero/toon'
SOURCE = OUT / 'hero.glb'
BANDS = (.68, .84, 1.0)
THRESHOLDS = (0, .28, .60)
...
def make_rig(mesh):
    ...
    rig = bpy.data.objects.new('Hero_Rig', arm); scene.collection.objects.link(rig)
    ...
    bone('neck', (0, 0, .505), (0, 0, .555), 'chest')
    bone('head', (0, 0, .555), (0, 0, .92), 'neck')
    for side, sign in [('L', 1), ('R', -1)]:
        bone('clavicle.' + side, (sign * .08, 0, .48), (sign * .19, 0, .455), 'chest')
        bone('upper_arm.' + side, (sign * .19, 0, .455), (sign * .25, -.01, .365), 'clavicle.' + side)
        bone('forearm.' + side, (sign * .25, -.01, .365), (sign * .30, -.025, .305), 'upper_arm.' + side)
        bone('hand.' + side, (sign * .30, -.025, .305), (sign * .323, -.04, .272), 'forearm.' + side)
        bone('thigh.' + side, (sign * .105, 0, .245), (sign * .12, -.01, .132), 'pelvis')
        bone('shin.' + side, (sign * .12, -.01, .132), (sign * .126, .005, .048), 'thigh.' + side)
        bone('foot.' + side, (sign * .126, .005, .048), (sign * .126, -.10, .032), 'shin.' + side)
    ...
    mesh.parent = rig
    mod = mesh.modifiers.new('Hero Skin', 'ARMATURE'); mod.object = rig
    ...
    for vertex in mesh.data.vertices:
        ...
        torso = blend(torso, {'head': 1}, smooth(.535, .55, z))
        arm_weights = blend({'upper_arm.' + side: 1}, {'forearm.' + side: 1}, 1 - smooth(.351, .381, z))
        arm_weights = blend(arm_weights, {'hand.' + side: 1}, 1 - smooth(.295, .315, z))
        arm_weights = blend(arm_weights, {'clavicle.' + side: 1}, smooth(.417, .445, z))
        arm_amount = smooth(.155 + max(0, .455 - z) * .56, .185 + max(0, .455 - z) * .56, ax)
        arm_amount *= smooth(.23, .265, z) * (1 - smooth(.535, .59, z))
        leg = blend({'thigh.' + side: 1}, {'shin.' + side: 1}, 1 - smooth(.112, .137, z))
        leg = blend(leg, {'foot.' + side: 1}, 1 - smooth(.07, .095, z))
        ...
        # The flared side edges wrap forward of the back panel, outside the legs.
        side_hem = smooth(.185, .20, ax) * (1 - smooth(.245, .28, z)) * smooth(0, .012, y)
        cape_amount = max(cape_amount, side_hem)
        # This hero's low cape hem overlaps boot height. Its entire back panel
        # stays on the cape chains, including the bottom edge during a stride.
        cape_amount *= 1 - smooth(.475, .52, z)
        result = blend(result, cape, cape_amount)
        if z >= .55: result = {'head': 1.0}  # Rigid face, ears and every hair spike.
    ...
    rig['preservation'] = 'Rigid face and hair. No subdivision, decimation, remesh, UV edit or texture repaint.'
    return rig
    ...
    for clip, period in PERIODS.items():
        ...
        for frame in range(1, period + 2):
            ...
            for side, sign, offset in [('L', 1, 0), ('R', -1, .5)]:
                ...
                ankle = Vector((sign * .126, fy, .048 + lift))
                ...
                upper_dir = Vector((sign * .55, swing, -.83)).normalized()
                fore_dir = Vector((sign * .62, swing - (.20 if running else .04), -.78)).normalized()
...
def validate(mesh, rig, before, source_hash, base_hash):
    ...
    head = [v for v in mesh.data.vertices if v.co.z >= .55]
    assert all(len(v.groups) == 1 and v.groups[0].group == head_group and v.groups[0].weight == 1 for v in head)
    result = {'source_sha256': source_hash, 'base_color_sha256': base_hash, 'geometry_before': before,
              ...
              'rigid_head_vertices': len(head), 'normal_angle_degrees': 32, 'normal_map': False, 'subdivision': False,
...
def export_game(mesh, rig, outline, matte, base_hash):
    ...
    try:
        bpy.ops.export_scene.gltf(filepath=str(OUT / 'hero_toon.glb'), export_format='GLB', use_selection=True, use_active_scene=True,
    ...
    blob = (OUT / 'hero_toon.glb').read_bytes(); size = struct.unpack_from('<I', blob, 12)[0]
...
def render_previews():
    scene = bpy.data.scenes['Toon']; rig = bpy.data.objects['Hero_Rig']
...
def main():
    ...
    mesh = next(o for o in scene.objects if o.type == 'MESH'); mesh.name = 'Hero_Body'
    ...
    # Replacing the copy's mesh with the unrigged source resets its groups.
    assert len(original_obj.vertex_groups) == 0
    ...
    scene['source'] = 'Input: assets/characters/tripo_hiro/hero.glb; exporter metadata: Tripo. Original GLB is unchanged.'
    scene['normal_policy'] = '32 degree angle-limited normals; no normal/bump maps or subdivision. Rigid face and hair weights.'
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(OUT / 'hero_toon.blend'))
