...
func setup(player_units: Array[BattleUnit], job_database: JobDatabase, skill_database: SkillDatabase, unlock_system: SkillUnlockSystem) -> void:
...
func _populate_jobs(selector: OptionButton, selected_id: String, excluded_id: String = "") -> void:
	...
	for job_id in selected_unit.unlocked_job_ids:
		...
		selector.add_item(job.job_name if job else job_id)
...
func _ensure_distinct_jobs(unit: BattleUnit) -> void:
	...
	for candidate in unit.unlocked_job_ids:
		if candidate != unit.main_job_id:
			unit.sub_job_id = candidate
			var job := jobs.get_job(candidate)
			unit.sub_job_name = job.job_name if job else candidate
			return
	unit.sub_job_id = ""
	unit.sub_job_name = "None"
...
func _refresh_skills() -> void:
	...
	for skill_id in unlocks.get_available_skill_ids_for_unit(selected_unit):
		...
		button.pressed.connect(func() -> void:
			if selected_unit.equip_skill(skill_id):
				_refresh_skills()
			else:
				message_label.text = "Max 6 skills can be equipped")
		available_list.add_child(button)
	message_label.text = "Equipped Skills %d / %d" % [selected_unit.equipped_skill_ids.size(), BattleUnit.MAX_EQUIPPED_SKILLS]
