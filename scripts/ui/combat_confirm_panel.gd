...
func open(attacker: BattleUnit, target: BattleUnit, preview: Dictionary) -> void:
	preview_label.text = "Attacker: %s\nTarget: %s\n\nDamage: %d  Hit Rate: %d%%\nTarget HP: %d / %d\nAfter HP: %d / %d\nDirection: %s\nTerrain: %s\nLine of Sight: %s\nDefeat: %s" % [attacker.unit_name, target.unit_name, preview.damage, preview.hit_rate, target.hp, target.max_hp, preview.after_hp, target.max_hp, preview.direction, preview.terrain, preview.line_of_sight, "Yes" if preview.after_hp == 0 else "No"]
	visible = true
