extends SceneTree
## Exercise the imported skin and the actual sample's locomotion controller.
...
func run() -> void:
	...
	check(actor.model_scene == world.player_model and actor.model_scene.resource_path.ends_with("meshy_hero2/hero2.glb"), "Sample spawns the hero2 GLB")
	...
	var mesh := actor.model.find_child("Mesh_0", true, false) as MeshInstance3D
	var material := mesh.get_active_material(0) as ShaderMaterial
	check(mesh.skin != null and material != null, "Skin and Toon ShaderMaterial imported")
	if material == null:
		quit(1)
		return
	check(material.get_shader_parameter("base_color_texture") is Texture2D, "Original Base Color texture retained")
	check(actor.model.get_meta("hero2_toon_version", 0) == 1, "GLB post-import applied the Toon look")
	var outline := actor.model.find_child("Toon_Outline", true, false) as MeshInstance3D
	check(outline != null and outline.skin != null and outline.get_node(outline.skeleton) == skeleton and mesh.get_node(mesh.skeleton) == skeleton, "Thin outline follows the same skeleton")
	check(outline != null and outline.material_override is ShaderMaterial and outline.material_override.shader.resource_path.ends_with("hero2_outline.gdshader"), "Baked outline uses its own unshaded material")
	player.play("idle", 0)
	player.seek(0, true)
	player.advance(0)
	...
	world._process(0.01)
	actor._process(0)
	# Speed is derived from float32 world-position deltas; allow subpixel rounding.
	check(player.current_animation == "run" and absf(player.speed_scale - 1.25) < 0.001, "Shift selects run near 1.25x (actual: %.6f)" % player.speed_scale)
	...
	if "--capture" in OS.get_cmdline_user_args():
		...
		for clip in ["idle", "walk", "run"]:
			...
			root.get_texture().get_image().save_png("res://artifacts/meshy_hero2/toon/jrpg_hero2_" + clip + ".png")
	world.queue_free()
	await process_frame
