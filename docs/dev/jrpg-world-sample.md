...
| LT / RT | 縮小 / 拡大 | 縮小 / 拡大 |
| LB | 割り当てなし（時間帯変更を解除） | カメラを90度回転 |
| 十字キー | 上：全景、右：町、下：高地、左：橋 | マス・メニュー選択 |
| Back / Share | 探索UIの表示切替 | — |
| Vキー / X（PS系では□） | 主人公の2D・3D切替 | 主人公の2D・3D切替 |
...
主人公は `assets/characters/base/base_1.bbmodel` の表示中の本体を使用します。隠されている武器は出力しません。NPC・敵は元の2D表示です。

- 通常移動：元モデルの `animation.walk_mcp_test` を `walk` として再生。
- 走行：元モデルの `animation.run` を `run` として再生。
...
- 停止：元モデルの静止姿勢。移動方向に合わせて3Dモデルを回転。
- `V` / `X・□` で2Dと3Dを相互に切り替え。探索・会話・戦闘中も位置やHPを保持。
- 初めから2Dで起動する場合、シーンルートのInspectorで `Use 3d Player` をオフにする。ゲーム中の切り替えは保存しません。

![主人公の静止・歩行・走行と元の2D](../../assets/world_jrpg/explorer_preview.png)

GLBの再生成（Python標準ライブラリのみ）：
