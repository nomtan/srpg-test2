...
- `cliff_corner` / `cliff_corner_outer` / `cliff_corner_inner` は `_create_cliff_sides()` から
  一度も参照されておらず、現行コードでは未使用と確認した（4.の判断どおり本Stepの対象外）。
  凸角の隙間が目視で気になる場合のみ、後続Stepで対応を検討する。
- `MapCellVisualData` に向き情報が無いことを確認した。階段は現行フォールバックと同じ
  固定向き（低い側=北/-Z、高い側=南/+Z）のまま本番アセット化した。向き対応は課題として残る。
