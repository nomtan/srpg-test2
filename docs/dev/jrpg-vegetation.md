...
メッシュ・材質を共有し、バリエーション・LOD・チャンクごとにMultiMeshへまとめる。草自体の影は無効。

## 距離別LOD

`natural_vegetation.gd` の `TREE_DISTANCES` / `GRASS_DISTANCES` で調整する。
距離はGodotのワールド単位で、カメラからMultiMeshのAABB中心まで。個々の木・草からの距離ではない。

| 対象 | 近景 | 中景 | 遠景 |
| --- | --- | --- | --- |
| 木 | 0〜45：従来の詳細モデル・影あり | 45〜110：葉を間引き、残す葉を拡大・影なし | 110〜：不透明な立体樹冠・影なし |
| 草 | 0〜30：従来の形状と密度 | 30〜55：6枚葉の株、株数は約半分 | 55〜：非表示 |

広葉樹は7,552→3,184→740三角形、針葉樹は19,194→5,586→2,058三角形／本。
通常草は95→30三角形／株で、中景では配置数も減る。
同じチャンクの各LODに共通のAABBを設定し、株数の変化で切替距離がずれて穴が開くことを防ぐ。
近景の配置・メッシュ生成は従来どおり。全LODで材質を共有するため、濡れ色・積雪・戦闘中の樹冠非表示は引き続き反映される。

Compatibilityでも使える不透明な即時切替を採用。境界では形状・密度・影の切替が見える場合がある。
全景では立体樹冠で森の輪郭を残すため、従来より樹冠が詰まって見える。
地形の面結合とチャンクの読み込み・解放は今回の変更に含めず、全域の起動時生成は従来どおり。

### LOD検証

Godot 4.6.1 / GL Compatibility / RTX 4050 Laptop / 1280×720 / 昼・晴れ。
同一プロセス内でLODを無効にした従来相当の描画（木は全距離詳細、草85）と比較。
VSyncを無効化し、各視点45フレーム待機後の90フレームの平均を測定した。数値はこの環境の参考値。

| 視点 | 従来相当 | LODあり |
| --- | --- | --- |
| 橋（4） | 約6.6ms/フレーム | 約3.5ms/フレーム |
| 山（3） | 約13.7ms/フレーム | 約3.6ms/フレーム |
| 全景（1） | 約34.3ms/フレーム | 約8.2ms/フレーム |

`verify_vegetation_lod.gd` は形状削減量・配置維持・共通材質・切替範囲を検査する。
`--render` を付けると実描画比較と `.godot/vegetation-lod/` への画像出力も行う。
移動・会話・戦闘・全16通りの時間帯／天候は既存の `verify_sample.gd` で確認。

```powershell
godot --headless --path . --log-file ./.godot/vegetation-lod-check.log --script scripts/world_jrpg/verify_vegetation_lod.gd
godot --path . --log-file ./.godot/vegetation-lod-render.log --resolution 1280x720 --script scripts/world_jrpg/verify_vegetation_lod.gd -- --render
```
