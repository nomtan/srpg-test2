...
const TOON = preload("res://assets/characters/tripo_knight/knight_toon.gdshader")
const OUTLINE = preload("res://assets/characters/tripo_knight/knight_outline.gdshader")
...
func _post_import(scene: Node) -> Object:
	for mesh: MeshInstance3D in scene.find_children("*", "MeshInstance3D", true, false):
		...
		for surface in mesh.mesh.get_surface_count():
			...
			if source == null or source.albedo_texture == null:
				push_error("Tripo knight requires its original Base Color texture")
				return null
	...
	scene.set_meta("knight_toon_version", 1)
	return scene
