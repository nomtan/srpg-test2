@tool
class_name AshenMCP_WebSocketServer
extends Node
...
func _connect_pending(p: PendingPeer) -> bool:
	if p.ws != null:
		...
		if p.tcp.get_status() != StreamPeerTCP.STATUS_CONNECTED:
			...
			p.ws.inbound_buffer_size = 16 * 1024 * 1024 # 16 MB
			p.ws.outbound_buffer_size = 16 * 1024 * 1024 # 16 MB
			p.ws.max_queued_packets = 2048
