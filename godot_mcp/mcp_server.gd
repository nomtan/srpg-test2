...
var base_port := 9080
var port := 9080        # Actual bound port (may differ from base_port if taken)
var max_port_scan := 5  # Try 9080..9084
...
var mcp_panel = null        # The MCP Status panel showing connection info
var initialization_sent := {}  # Track which clients have received initialization
...
func _enter_tree():
	...
	# Start WebSocket server — try base_port..base_port+max_port_scan-1
	var bound := false
	for try_port in range(base_port, base_port + max_port_scan):
		var err = tcp_server.listen(try_port)
		if err == OK:
			port = try_port
			bound = true
			break
		else:
			print("Port %d taken (err %d), trying next..." % [try_port, err])
	
	if bound:
		print("MCP WebSocket listening on port %d" % port)
		set_process(true)
	else:
		printerr("Failed to bind any port from %d to %d" % [base_port, base_port + max_port_scan - 1])
	
	
	# Add MCP Status Panel to bottom UI
	print("Loading MCP Status Panel...")
	var mcp_scene_path = "res://addons/godot_mcp/ui/mcp_panel.tscn"
	if ResourceLoader.exists(mcp_scene_path):
		var mcp_scene = load(mcp_scene_path)
		mcp_panel = mcp_scene.instantiate()
		mcp_panel.websocket_server = self  # Share server reference
		add_control_to_bottom_panel(mcp_panel, "MCP Status")
	else:
		printerr("Failed to load MCP Status Panel scene")
...
# ---------------------------------------------------------------------------
# Status helpers — used by mcp_panel.gd
# ---------------------------------------------------------------------------
		...
		if client.state == -1: # Handshaking
			if client.ws != null:
				...
				if ws_state == WebSocketPeer.STATE_OPEN:
					...
					var init_text = JSON.stringify(init_msg)
					client.ws.send_text(init_text)
					print("[Client ", id, "] Sent MCP initialization message")
